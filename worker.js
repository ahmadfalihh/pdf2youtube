...
async function main() {
  ...
  try {
    ...
    await uploadToDrive(outputVideoPath, finalVideoName, 'video/mp4', targetFolderId);
    ...
    console.log(`✅ BERHASIL! Video tayang di: https://youtu.be/${ytVideoId}`);
...
async function generateScript(imagePath, style, apiKey) {
  const openai = new OpenAI({ apiKey: apiKey, baseURL: "https://litellm.koboi2026.biz.id/v1" });
  const imageData = fs.readFileSync(imagePath).toString("base64");
  const prompt = `Anda narator profesional. Jelaskan materi di GAMBAR SLIDE ini secara langsung.\nGaya: ${style}.\nPanjang: 70-100 kata.\nBahasa: Indonesia.`;
  
  const response = await openai.chat.completions.create({
    model: LLM_MODEL,
    messages: [ { role: "user", content: [ { type: "text", text: prompt }, { type: "image_url", image_url: { url: `data:image/jpeg;base64,${imageData}` } } ] } ]
  });
  return response.choices[0].message.content;
}

function mapVoiceToOpenAI(geminiVoice) {
  const voiceMap = { 'Aoede': 'nova', 'Callirrhoe': 'shimmer', 'Kore': 'nova', 'Leda': 'shimmer', 'Autonoe': 'nova', 'Zephyr': 'echo', 'Puck': 'fable', 'Fenrir': 'onyx', 'Orus': 'onyx', 'Charon': 'onyx', 'Enceladus': 'echo' };
...
async function generateAudio(text, voiceName, outputPath, apiKey) {
  ...
  const response = await openai.audio.speech.create({ model: "tts-1", voice: openAiVoice, input: text });
...
async function generateYouTubeMetadata(fullScript, apiKey) {
  ...
  const response = await openai.chat.completions.create({ model: LLM_MODEL, messages: [{ role: "user", content: prompt }] });
...
async function renderVideo(slides, finalOutput, bgmPath) {
  ...
  for (let i = 0; i < slides.length; i++) {
    ...
    await new Promise((resolve, reject) => {
      ffmpeg().input(slides[i].image).loop().input(slides[i].audio).outputOptions(['-c:v libx264', '-tune stillimage', '-c:a aac', '-b:a 192k', '-pix_fmt yuv420p', '-shortest']).save(clipPath).on('end', resolve).on('error', reject);
    });
  ...
  await new Promise((resolve, reject) => {
    ffmpeg().input(concatListPath).inputOptions(['-f concat', '-safe 0']).outputOptions('-c copy').save(rawVideoPath).on('end', resolve).on('error', reject);
  });

  if (bgmPath) {
    await new Promise((resolve, reject) => {
      ffmpeg().input(rawVideoPath).input(bgmPath).complexFilter(['[1:a]volume=0.15[bgm]', '[0:a][bgm]amix=inputs=2:duration=first:dropout_transition=2[aout]']).outputOptions(['-map 0:v', '-map [aout]', '-c:v copy', '-c:a aac', '-b:a 192k']).save(finalOutput).on('end', resolve).on('error', reject);
    });
  } else {
...
async function uploadToYouTube(videoPath, seoData, thumbnailPath) {
  ...
  try { await youtube.thumbnails.set({ videoId: videoId, media: { body: fs.createReadStream(thumbnailPath) } }); } catch (e) {}
  return videoId;
}
